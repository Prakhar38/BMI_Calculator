...
class BMI_Calc(CTk):
    ...
    def __init__(self):
        super().__init__()
        self.geometry(f"{self.win_width}x{self.win_height}+{(self.winfo_screenwidth() // 2) - self.win_width // 2}+{(self.winfo_screenheight()//2) - self.win_height // 2}") # Size of tkinter Window
        self.title("BMI Calculator")  # Title of the Application
        self.resizable(0, 0) # Window would not be resizeable
        self._set_appearance_mode(mode_string="Light")
        self.wm_iconbitmap(r"Bin\CalcLogo.ico")
    ...
    def setElement(self):
        ...
        CTkOptionMenu(self, 200, 30, 5, variable=self.Gen, fg_color="#ffa500", values=["Male", "Female", "Other"], dropdown_fg_color="#ffa500").place(x=50, y=340)
        ...
        CTkOptionMenu(self, 200, 30, 5, variable=self.unit, fg_color="#ffa500", values=["Metric", "US Unit"], dropdown_fg_color="#ffa500", command=lambda text: self.click(text)).place(x=50, y=420)
    ...
    def createEntry(self, placeholder, x, y):
        entry = CTkEntry(self, 201, 30, 5, text_color="white", placeholder_text=placeholder, placeholder_text_color="white", fg_color="#ffa500")
        entry.place(x=x, y=y)
        return entry
    ...
    def About(self):
        msg.showinfo("About Me", "A BMI Calculator by Prakhar Singh")
